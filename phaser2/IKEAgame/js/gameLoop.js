...
function createText(){
    ...
    notifText = game.add.text(609.5, 394, 'notifText', { fontSize: '20px', fill: '#FFF' });
    chatStateString = "";
...
function delayMessage(seconds, imageTag, text){
	queue[queue.length] = new message(game, imageTag, text);
}
...
function gameCreate(){
	game.time.advancedTiming = true;
	    ...
	    graphics.beginFill(0xffffff, .5);//fcb6bd, .5);//4f3e8f, 0.5); //
	    ...
	    phone = game.add.sprite(575, 400, 'phonefront');
	    notif = game.add.sprite(605, 395, 'notif');
		...
		chats[0] = new chatChoice(game, "", .42, "Welcome to IKEA. The sun is setting. Why are you here?", new Array("Enter"), 'ikea-outside');
	    ...
	    //new elevator at night [[6 A's]]
	    //Ok, you got this. You begin to narrate in an ominous voice: On a dare, I are alone in IKEA at night, lights softly glowing, where the laws of gravity and time may or may not apply. Where should I go first?
	    ...
	    ////////////////////////////////////////////////////////////////////
	    //THE ONLY writing left is ^^ bedroom and storage and above,, and below
	    ...
	    //ending 2 (successful escape) 2.1 2.2 [[9 A's]]
	    ...
	    music.play();
	  	...
	  	for (var i = 0; i < bubbleLen; i++) {
	    	...
	    	bubbles[i] = new bubble(game, '', '', 650, 500-i*40);
    	}
...
function gameUpdate(){
	debugText.text = "press R to restart: " + chatStateString + " " + queue.length;
